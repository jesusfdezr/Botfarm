...
export async function GET() {
  const clientId = process.env.X_CLIENT_ID?.trim() || process.env.NEXT_PUBLIC_X_CLIENT_ID?.trim();
  ...
  const requestProfile = async (token: string) => {
    ...
    if (!payload.data) throw new Error('X no devolvio el perfil autenticado.');
  ...
  try {
    ...
    if (activeAccessToken) {
      try {
        ...
        lastProfileDetail = profileError?.message || 'X rechazo la consulta del perfil.';
      }
    }
    ...
    if (activeRefreshToken) {
      ...
      try {
        ...
        profile = createLimitedProfile(error?.message || 'X no permitio consultar el perfil con el token renovado.');
      }

      return NextResponse.json({
        ok: true,
        message: 'Sesion local refrescada e importada.',
        session: {
          clientId,
          redirectUri,
          connectedAt: Date.now(),
    ...
    if (lastProfileDetail) {
      return NextResponse.json({
        ok: true,
        message: 'Sesion local importada, pero X no permite consultar el perfil.',
        session: {
          clientId,
          redirectUri,
          connectedAt: Date.now(),
          ...
          profile: createLimitedProfile(lastProfileDetail),
        },
      });
    }

    throw new Error('No hay access token ni refresh token validos para X.');
  } catch (error: any) {
